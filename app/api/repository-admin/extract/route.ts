...
export async function POST(req: NextRequest) {
  ...
  if (!rawText && sourceUrl && !isSourceWithinRecencyWindow({ source_url: sourceUrl })) {
    return NextResponse.json(
      {
        error: `That source URL is outside the ${SOURCE_RECENCY_YEARS}-year collection window. Use a report from ${new Date().getFullYear() - SOURCE_RECENCY_YEARS} or later, or paste an excerpt manually.`,
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      workforceComposition: result.workforceComposition,
