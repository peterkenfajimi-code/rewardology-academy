/** Capture GCB Bank PLC published profile screenshot from live or local dev. */
import fs from "fs";
...
const slug = "gcb-bank-plc-gh";
const base = process.argv[2] ?? "https://rewardologyacademy.com";
const envLabel = base.includes("localhost") ? "localhost" : "rewardologyacademy.com";
...
async function main() {
  ...
  const file = path.join(outDir, `06-profile-gcb-${envLabel.replace(/\./g, "-")}.png`);
